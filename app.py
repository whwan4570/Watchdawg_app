...
- Reads from Databricks SQL warehouse
...
# Using Databricks data warehouse

# Data cache - stores all data in memory to minimize warehouse queries
_data_cache = None
_cache_timestamp = None
CACHE_TTL_SECONDS = 3600  # Refresh cache every hour

# Databricks connection manager (singleton with connection pooling)
class DatabricksConnectionManager:
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(DatabricksConnectionManager, cls).__new__(cls)
            import threading
            cls._lock = threading.Lock()
        return cls._instance
    
    def get_connection(self):
        """Get or create a reusable Databricks connection."""
        with self._lock:
    ...
    def _create_connection(self):
        """Create a new Databricks connection using token-based M2M authentication (no browser)."""
        import requests
        
        hostname = DATABRICKS_HOST.replace('https://', '').replace('http://', '')
        
        # Get M2M OAuth token without browser interaction
        ...
        if token_response.status_code != 200:
            ...
            raise Exception(f"Failed to get OAuth token: {token_response.status_code}")
        
        access_token = token_response.json()["access_token"]
        
        # Connect using the token (no browser popup!)
        ...
        print(f"✓ Created Databricks connection to {hostname} (M2M token auth)")
        return connection
...
_db_manager = DatabricksConnectionManager()

def load_all_data(force_refresh=False):
    """
    Load and cache ALL data in memory to minimize warehouse queries.
    
    Best Practice: Query the warehouse ONCE at startup, then do all filtering in pandas.
    This prevents hundreds of queries per user session and reduces warehouse costs.
    ...
    print(f"🔄 Loading data from Databricks warehouse...")
    
    # Query ALL data from Databricks ONCE with retry logic
    max_retries = 3
    retry_delay = 2
    ...
    for attempt in range(max_retries):
        try:
            query = f"""
              SELECT 
                TO_DATE(CONCAT(CAST(`Offense Year` AS INT), '-', LPAD(CAST(`Offense Month` AS INT), 2, '0'), '-', LPAD(CAST(`Offense Day` AS INT), 2, '0'))) as date,
                CAST(`Offense Time` as STRING) as time,
                HOUR(`Offense Time`) as hour,
                CONCAT(CAST(`Offense Year` AS INT), '-', LPAD(CAST(`Offense Month` AS INT), 2, '0'), '-', LPAD(CAST(`Offense Day` AS INT), 2, '0'), ' ', `Offense Time`) as datetime,
                `Offense Category` as offense,
                `Offense Sub Category` as offense_sub_category,
                `NIBRS Crime Against Category` as crime_against_category,
                `Block Address` as location,
              ...
              FROM {CATALOG}.{SCHEMA}.crime_gold_data
              WHERE `Latitude` IS NOT NULL AND `Longitude` IS NOT NULL
            """
            
            print(f"🔗 Connecting to Databricks warehouse (attempt {attempt + 1}/{max_retries})...")
            conn = get_connection()
            cursor = conn.cursor()
            print(f"📊 Executing query to fetch all data...")
            cursor.execute(query)
            result = cursor.fetchall_arrow().to_pandas()
            cursor.close()
            print(f"✅ Successfully loaded {len(result)} records from Databricks")
            break
            ...
            if attempt < max_retries - 1:
                ...
                print(f"❌ All retry attempts failed. Returning empty dataset.")
    ...
    print(f"💾 Cache will expire in {CACHE_TTL_SECONDS/60:.0f} minutes")
    
    return result.copy()


def get_connection():
    """Get the managed Databricks connection (for backward compatibility)."""
    return _db_manager.get_connection() if _db_manager else None
            ...
            children=[
                dbc.Card(
                    dbc.CardBody([
                        ...
                        html.Hr(),
                        html.Label("🔄 Data Cache", className="fw-semibold mb-2"),
...
kpi_cards = dbc.Row(
    [
        ...
        dbc.Col(
            dbc.Card(
                dbc.CardBody([
                    html.H6("Against Property", className="text-muted"),
                    dcc.Loading(
        ...
        dbc.Col(
            dbc.Card(
                dbc.CardBody([
                    html.H6("Against Person", className="text-muted"),
                    dcc.Loading(
        ...
        dbc.Col(
            dbc.Card(
                dbc.CardBody([
                    html.H6("Against Society", className="text-muted"),
                    dcc.Loading(
                ...
                [
                    ...
                    dbc.Col(
                        [
                            ...
                            html.Label("🗺️ Crime Type Filter", className="fw-semibold mb-2"),
                            dcc.Loading(
...
def display_cache_info(_, refresh_count):
    """Display information about the data cache."""
    if _cache_timestamp is None:
        return html.Div([
            html.I(className="bi bi-exclamation-triangle text-warning me-1"),
            "Cache not loaded - check Databricks connection"
        ], className="small text-warning")
    
    if _data_cache is None or len(_data_cache) == 0:
        return html.Div([
            html.I(className="bi bi-exclamation-triangle text-warning me-1"),
            "No data in cache - check warehouse connection"
        ], className="small text-warning")
    ...
    if cache_age_minutes < 1:
        age_display = f"{int(cache_age_seconds)} seconds ago"
    elif cache_age_minutes < 60:
        age_display = f"{int(cache_age_minutes)} minutes ago"
    else:
        age_display = f"{int(cache_age_minutes/60)} hours ago"
    
    record_count = len(_data_cache) if _data_cache is not None else 0
    
    return [
        html.Div(f"📊 {record_count:,} records", className="small"),
        html.Div(f"🕐 Updated: {age_display}", className="small")
    ]
...
def refresh_data_cache(n_clicks, current_count):
    """Force refresh the data cache from Databricks."""
    try:
        if n_clicks:
            print("🔄 Forcing data cache refresh from Databricks...")
            # Force reload
            load_all_data(force_refresh=True)
            return current_count + 1
        return current_count
    except Exception as e:
        print(f"Error refreshing data cache: {e}")
        import traceback
        traceback.print_exc()
        return current_count
